<template>
<div class="container build">
    <div class="wrap">
        <div class="shop-wrap">
            <div class="shop-layer">

                <div class="weui-panel__hd">
                    <a href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg">
                        全部分类
                    </a>
                    <a href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg current">
                        分类1
                    </a>
                    <a href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg">
                        分类2
                    </a>
                    <a href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg">
                        分类3
                    </a>
                </div>
                <div class="weui-panel__bd">
                    <div href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg">
                        <div class="weui-media-box__hd">
                            <img class="weui-media-box__thumb"
                                 src="data:image/png;base64,"
                                 alt="">
                        </div>
                        <div class="weui-media-box__bd">
                            <h4 class="weui-media-box__title">标题一</h4>

                            <p class="weui-media-box__desc">已售20</p>

                            <div class="weui-media-box__info"><span>20元</span>

                                <p class="price-oper"><a href="#" class=""><i
                                        class="weui-icon-info-circle"></i> </a></p></div>
                        </div>
                    </div>
                    <!--item-->
                    <div href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg">
                        <div class="weui-media-box__hd">
                            <img class="weui-media-box__thumb"
                                 src="data:image/png;base64,"
                                 alt="">
                        </div>
                        <div class="weui-media-box__bd">
                            <h4 class="weui-media-box__title">标题一</h4>

                            <p class="weui-media-box__desc">已售20</p>

                            <div class="weui-media-box__info"><span>20元</span>

                                <p class="price-oper"><a href="#" class=""><i
                                        class="weui-icon-info-circle"></i> </a></p></div>
                        </div>
                    </div>
                    <!--item-->
                    <div href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg">
                        <div class="weui-media-box__hd">
                            <img class="weui-media-box__thumb"
                                 src="data:image/png;base64,"
                                 alt="">
                        </div>
                        <div class="weui-media-box__bd">
                            <h4 class="weui-media-box__title">标题一</h4>

                            <p class="weui-media-box__desc">已售20</p>

                            <div class="weui-media-box__info"><span>20元</span>

                                <p class="price-oper"><a href="#" class=""><i
                                            class="weui-icon-info-circle"></i> </a></p></div>
                        </div>
                    </div>
                    <!--item-->
                    <div href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg">
                        <div class="weui-media-box__hd">
                            <img class="weui-media-box__thumb"
                                 src="data:image/png;base64,"
                                 alt="">
                        </div>
                        <div class="weui-media-box__bd">
                            <h4 class="weui-media-box__title">标题二</h4>

                            <p class="weui-media-box__desc">已售20</p>

                            <div class="weui-media-box__info"><span>20元</span>

                                <p class="price-oper"><a href="#"><i class="weui-icon-circle"></i> </a><span>12</span>
                                    <a href="#" class=""><i
                                            class="weui-icon-info-circle"></i> </a></p></div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <p class="notic">
            <i class="weui-icon-info-circle"></i><span>当前页面为你客户看到的效果。</span>
        </p>
    </div>
    <!--footer-->
    <footer>
        <div class="weui-tabbar">

            <router-link to="ShopIndex" href="javascript:;" class="weui-tabbar__item weui-bar__item_on">
                <img src="../assets/img//icon_tabbar.png" alt="" class="weui-tabbar__icon">

                <p class="weui-tabbar__label">首页</p>
            </router-link>
            <router-link href="javascript:;" class="weui-tabbar__item" to="EditProduct">
                    <span style="display: inline-block;position: relative;">
                        <img src="../assets/img//icon_tabbar.png" alt="" class="weui-tabbar__icon">
                        <span class="weui-badge weui-badge_dot" style="position: absolute;top: 0;right: -6px;"></span>
                    </span>

                <p class="weui-tabbar__label">小铺中心</p>
            </router-link>
        </div>
    </footer>
</div>
</template>

<script>
    export default {
      name: 'ProductIndex',
      data() {
          return {

          }
      },
      created: function() {

      }
    }
</script>

<style lang="less" scoped>
    @import '../assets/less/toc-index.less';
    @import '../assets/less/tob-build.less';
</style>
